...
async function run() {
  ...
  check(fs.existsSync(jsFile), 'workflow-studio.js frontend exists');
  ...
  check(stateContent.includes("'workflow-studio'"), 'state.js has workflow-studio tab entry');

  const indexContent = fs.readFileSync(path.join(ROOT, 'public/dashboard/index.html'), 'utf8');
  check(indexContent.includes('data-tab="workflow-studio"'), 'index.html has workflow-studio sidebar entry');

  const swContent = fs.readFileSync(path.join(ROOT, 'public/dashboard/service-worker.js'), 'utf8');
  check(swContent.includes('/dashboard/workflow-studio.js'), 'service-worker.js has workflow-studio.js');

  console.log('\n--- Workflow Studio Dashboard API: ' + passed + ' passed, ' + failed + ' failed ---');
