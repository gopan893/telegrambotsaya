'use strict';

module.exports = {
  agentRegistry: require('./agent-registry'),
  agentMemoryRelevance: require('./agent-memory-relevance'),
  agentMemoryStore: require('./agent-memory-store'),
  agentActionDetector: require('./agent-action-detector'),
  agentActionPlan: require('./agent-action-plan'),
  agentActionMapper: require('./agent-action-mapper'),
  agentApprovalFlow: require('./agent-approval-flow'),
  agentEvaluationCases: require('./agent-evaluation-cases'),
  agentEvaluationHarness: require('./agent-evaluation-harness'),
  agentEvaluationScorer: require('./agent-evaluation-scorer'),
  agentExecutorBridge: require('./agent-executor-bridge'),
  agentExecutorPolicy: require('./agent-executor-policy'),
  agentPersonality: require('./agent-personality'),
  agentPreferences: require('./agent-preferences'),
  agentProfileStore: require('./agent-profile-store'),
  agentPromptComposer: require('./agent-prompt-composer'),
  agentResponseRenderer: require('./agent-response-renderer'),
  agentRouter: require('./agent-router'),
  agentScoring: require('./agent-scoring'),
  agentStyleBuilder: require('./agent-style-builder'),
  conversationBus: require('./conversation-bus'),
  councilEngine: require('./council-engine'),
  councilMemory: require('./council-memory'),
  councilModerator: require('./council-moderator'),
  councilStore: require('./council-store'),
  debateEngine: require('./debate-engine'),
  advancedRiskScorer: require('./advanced-risk-scorer'),
  executorPreflightReview: require('./executor-preflight-review'),
  executorResultRouter: require('./executor-result-router'),
  agentAssignment: require('./agent-assignment'),
  agentConflictDetector: require('./agent-conflict-detector'),
  agentResultAggregator: require('./agent-result-aggregator'),
  agentTaskQueue: require('./agent-task-queue'),
  agentTaskRunner: require('./agent-task-runner'),
  agentTaskStore: require('./agent-task-store'),
  confidenceScorer: require('./confidence-scorer'),
  decisionDetector: require('./decision-detector'),
  decisionSynthesis: require('./decision-synthesis'),
  decisionStore: require('./decision-store'),
  delegationEngine: require('./delegation-engine'),
  delegationMemory: require('./delegation-memory'),
  handoffManager: require('./agent-handoff-manager'),
  learningNotes: require('./agent-learning-notes'),
  optionExtractor: require('./option-extractor'),
  proposalBuilder: require('./proposal-builder'),
  prosConsEngine: require('./pros-cons-engine'),
  responsePolicy: require('./response-policy'),
  riskDetector: require('./risk-detector'),
  tradeoffAnalyzer: require('./tradeoff-analyzer'),
  topicClassifier: require('./topic-classifier'),
  utils: require('./agent-utils')
};
