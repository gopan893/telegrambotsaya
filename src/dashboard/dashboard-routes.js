...
function registerDashboardRoutes(app, rawServices = {}) {
  ...
  try {
    ...
    privacyRoutes.registerPrivacyRoutes(router, services);
  } catch (e) {
    const log = services.logger || console;
    log.warn('[dashboard] Privacy routes skipped:', e.message);
  }

  try {
    const releaseCandidateRoutes = require('./release-candidate-routes');
    releaseCandidateRoutes.registerReleaseCandidateRoutes(router, services);
  } catch (e) {
    const log = services.logger || console;
  ...
  try {
    ...
    operatorRoutes.registerOperatorRoutes(router, services);
  } catch (e) {
    const log = services.logger || console;
  ...
  try {
    ...
    boundaryRoutes.registerBoundaryRoutes(router, services);
  } catch (e) {
    const log = services.logger || console;
    log.warn('[dashboard] Boundary routes skipped:', e.message);
  }

  try {
    const performanceRoutes = require('./performance-routes');
    performanceRoutes.registerPerformanceRoutes(router, services);
  } catch (e) {
    const log = services.logger || console;
    log.warn('[dashboard] Performance routes skipped:', e.message);
  }

  try {
    const v2ReleaseRoutes = require('./v2-release-routes');
    v2ReleaseRoutes.registerV2ReleaseRoutes(router, services);
  } catch (e) {
    const log = services.logger || console;
