...
const CicdUI = (() => {
  async function loadReleases(targetEl) {
    const res = await Api.get('/api/dashboard/cicd/releases');
    if (!res.ok) return;
  ...
  async function loadProposals(targetEl) {
    const res = await Api.get('/api/dashboard/cicd/proposals');
    if (!res.ok) return;
  ...
  async function loadPipelines(targetEl) {
    const res = await Api.get('/api/dashboard/cicd/pipelines');
    if (!res.ok) return;
